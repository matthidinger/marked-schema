    ...
    if (type === 'array') {
        ...
        if (property.itemTypes && defined(property.itemTypes[0])) {
            ...
            if(property.itemTypes.length === 1) {
                t = property.itemTypes[0];
            }
            else {
                var allTypes = property.itemTypes.map(it => it[0]);
                var rootTypes = allTypes.map(it => it.split(".")[0]);

                // Action.Submit, Action.OpenUrl => "Action[]"
                if(rootTypes.every((val, i, arr) => val === arr[0]))
                {
                    t = rootTypes[0];
                }
                // TextBlock, Image => "array[]"
            }
